...
function setupOnlineOfflineListeners(visitorId: string) {
  ...
  const updateOnlineStatus = async (isOnline: boolean) => {
    ...
    try {
      ...
      if (!docSnap.exists()) {
        console.log(
          "[OnlineTracking] Visitor document not found, skipping online status update"
        );
        return;
      }
...
function setupActivityTracker(visitorId: string) {
  ...
  const updateActivity = async () => {
    ...
    try {
      ...
      if (!docSnap.exists()) {
        console.log(
          "[OnlineTracking] Visitor document not found, skipping activity update"
        );
        return;
      }
...
export async function updateVisitorPage(
  ...
  try {
    ...
    if (!docSnap.exists()) {
      console.log(
        "[OnlineTracking] Visitor document not found, skipping update"
      );
      return;
    }
...
export async function saveFormData(
  ...
  try {
    ...
    if (!docSnap.exists()) {
      console.log(
        "[OnlineTracking] Visitor document not found, skipping form data save"
      );
      return;
    }
...
export async function checkRedirectPage(
  visitorId: string
): Promise<string | null> {
  if (!db) return null;
  try {
...
export async function clearRedirectPage(visitorId: string) {
  ...
  try {
    ...
    if (!docSnap.exists()) {
      console.log(
        "[OnlineTracking] Visitor document not found, skipping redirect clear"
      );
      return;
    }
...
export async function setRedirectPage(visitorId: string, targetPage: string) {
  ...
  try {
    ...
    if (!docSnap.exists()) {
      console.log(
        "[OnlineTracking] Visitor document not found, skipping redirect set"
      );
      return;
    }
